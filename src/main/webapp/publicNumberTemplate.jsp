...
<script type="text/javascript">
	...
	$(function(){
		...
		$("#import_btn").click(function() {
			...
			if(filename==""){
				...
				url = "publicNumberTemplateServlet?opType=importFile";
				importFile();
			}
	    });
		...
		$("#export_btn").click(function() {
			$.messager.confirm('确认对话框', '确认下载?', function(r){
				if (r){
					url = "publicNumberTemplateServlet?opType=exportFile&filename=publicNumberTemplate.xls";
					exportFile();
					/* 
		...
		$("#dg").datagrid({
			idField:"id",
			url:'publicNumberTemplate/queryByPage',
			fitColumns:true,
			striped:true,
			pagination:true,
			rownumbers:true,
			...
			columns:[[
				...
				{field:'tId',width:100,align:'center',title:'模版编号'}
			]],
			toolbar:[{
				text:"新增",
				iconCls:"icon-add",
	...
	function remove() {
		...
		$.messager.confirm("提示","请确定要删除所选的【"+array.length+"】条记录吗？",function(yes){
			if(yes){
				...
				$.ajax({
					url:"publicNumberTemplate/deleteMore",
					type:"post",
					data:"ids="+ids,
					dataType:"json",
	...
	function update() {
		...
		url = "publicNumberTemplate/update?id="+array[0].id;
		openDialogForm();
	...
	<div id="dd" class="easyui-dialog" style="width: 500px;" closed="true">
		<form id="Form" method="post">
			<table style="margin: 0 auto;">
				...
				<tr>
					<td class="right">模版编号：</td>
					<td><input name="tId" class="easyui-textbox" data-options="required:true"></td>
				</tr>
			</table>
		</form>
	</div>
