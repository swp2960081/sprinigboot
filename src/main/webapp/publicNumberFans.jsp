...
<script type="text/javascript">
	...
	$(function(){
		...
		$("#import_btn").click(function() {
			...
			if(filename==""){
				...
				url = "publicNumberFansServlet?opType=importFile";
				importFile();
			}
	    });
		...
		$("#export_btn").click(function() {
			$.messager.confirm('确认对话框', '确认下载?', function(r){
				if (r){
					url = "publicNumberFansServlet?opType=exportFile&filename=publicNumberFans.xls";
					exportFile();
					/* 
		...
		$("#dg").datagrid({
			idField:"id",
			url:'publicNumberFans/queryByPage',
			fitColumns:true,
			striped:true,
			pagination:true,
			rownumbers:true,
			...
			columns:[[
				...
				{field:'pId',width:100,align:'center',title:'公众号'},
				{field:'addFans',width:100,align:'center',title:'新增粉丝数'},
				{field:'reduceFans',width:100,align:'center',title:'减少粉丝数'},
				{field:'countDate',width:100,align:'center',title:'统计日期'}
			]],
			toolbar:[{
				text:"新增",
				iconCls:"icon-add",
	...
	function remove() {
		...
		$.messager.confirm("提示","请确定要删除所选的【"+array.length+"】条记录吗？",function(yes){
			if(yes){
				...
				$.ajax({
					url:"publicNumberFans/deleteMore",
					type:"post",
					data:"ids="+ids,
					dataType:"json",
	...
	function update() {
		...
		url = "publicNumberFans/update?id="+array[0].id;
		openDialogForm();
	...
	<div id="dd" class="easyui-dialog" style="width: 500px;" closed="true">
		<form id="Form" method="post">
			<table style="margin: 0 auto;">
				...
				<tr>
					<td class="right">新增粉丝数：</td>
					<td><input name="addFans" class="easyui-numberbox" data-options="required:true"></td>
				</tr>
				<tr>
					<td class="right">减少粉丝数：</td>
					<td><input name="reduceFans" class="easyui-numberbox" data-options="required:true"></td>
				</tr>
				<tr>
					<td class="right">统计日期：</td>
					<td><input name="countDate" class="easyui-datebox" data-options="required:true,editable:false"></td>
				</tr>
			</table>
		</form>
	</div>
