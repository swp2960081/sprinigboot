...
<script type="text/javascript">
	...
	$(function(){
		...
		$("#import_btn").click(function() {
			...
			if(filename==""){
				...
				url = "userPublicNumberServlet?opType=importFile";
				importFile();
			}
	    });
		...
		$("#export_btn").click(function() {
			$.messager.confirm('确认对话框', '确认下载?', function(r){
				if (r){
					url = "userPublicNumberServlet?opType=exportFile&filename=userPublicNumber.xls";
					exportFile();
					/* 
		...
		$("#dg").datagrid({
			idField:"id",
			url:'userPublicNumber/queryByPageVo',
			fitColumns:true,
			striped:true,
			pagination:true,
			rownumbers:true,
			...
			columns:[[
				...
				{field:'userId',width:100,align:'center',title:'用户编号'},
				{field:'userName',width:100,align:'center',title:'用户帐号'},
	...
	function add() {
		...
		openDialogForm();
		$("#dd").dialog("setTitle","新增公众号与用户绑定信息");
	}
	...
	function update() {
		...
		openDialogForm();
		$("#dd").dialog("setTitle","修改公众号与用户绑定信息");
		//$("#pwd").textbox("disable");
...
<!-- 
	<div style="height: 5%;">
		<form id="searchForm" method="post" style="padding-top: 4px;" enctype="multipart/form-data">
			...
			帐&emsp;号：<input name="quserPublicNumbername" class="easyui-textbox" data-options="iconCls:'icon-man'">&emsp;
